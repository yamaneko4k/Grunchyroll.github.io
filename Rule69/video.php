...
$sql = "SELECT * FROM videos ORDER BY uploaded_on DESC limit $start_from,$num_per_page";
  ...
  <body class="background">
    ...
    <div class="item">
      <div class="order"> 
          <?php while($row = mysqli_fetch_array($rs_result)){ ?>
            <div class="crop_img">   
              <?php echo "<a title=".$row["uploader_username"]." href=\"display_video.php?id={$row['id']}\"</a>"; ?><img  id="img01" src="<?php echo 'database/thumbnails/'.$row["thumbnail_name"]; ?>" loading="lazy" ></a>
    ...
    <div class="center">
      ...
      <?php if (ceil($total_pages / $num_per_page) > 0): ?>
        <?php if ($page > 1): ?>
        <a href="video.php?page=<?php echo $page-1 ?>">Prev</a>
        <?php else: ?>
          <a href="video.php?page=<?php echo $page=1 ?>">Prev</a>
        <?php endif; ?>
        ...
        // }
        echo "<a class='pagination' href='video.php?page=".$page."'>".$page." / ".$total_pages."</a>"; 
        $_SESSION['video_current_page'] = $page;
        ?>

        <?php if ($page >= $total_pages): ?>
          <a class='active' href="video.php?page=<?php echo $page=$total_pages ?>">Next</a>
        <?php else: ?>
          <a href="video.php?page=<?php echo $page+1 ?>">Next</a>
        <?php endif; ?>     
      </div>
    </div>
    <?php endif; 
    if(mysqli_num_rows($rs_result) > 0){
      ...
      echo '<p class="nothing"><strong>nothing yet :(</strong></p>';
    } 
